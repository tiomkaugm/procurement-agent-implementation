...
## Penyederhanaan
- Revisi termin: di laporan diteruskan ke DA; di sini menjadi keputusan SLM dengan peluang diterima. Log dan dashboard menampilkan langkah ini sebagai serah terima ke DA (`event="term_request"` dari SLM lalu `event="term_response"` beratribut `agent="DA"`) supaya jejaknya cocok dengan narasi laporan. Ini murni pelabelan tampilan: tidak ada giliran/aksi DA yang sesungguhnya (tidak ada `env.step` terpisah, tidak ada pengaruh kebijakan DA) — rumus dan mekanismenya tetap sama seperti sebelumnya: undian `_p_termin(vendor)` dipicu di dalam langkah SLM. Lihat `_negotiate_terms` di `env.py` dan `describe()` di `presentation.py`.
...
## Penghentian dini kebijakan deterministik dan demo enam tahap terpisah (bug.md butir 2)
- **`ProcurementEnv` produksi memperoleh jalur berhenti baru: `stop_on_repeat`.** Jika rencana dan keadaan relevan (batch, kas terkomit, relasi vendor, harga disepakati, jenis pelanggaran — lihat `signature` di `_end_round`) sama persis dengan putaran sebelumnya, putaran berakhir dengan `stop_reason="no_progress"` alih-alih menghabiskan sisa `max_rounds`. Diaktifkan lewat opsi `reset(options={"stop_on_repeat": ...})`; `RuleBasedPolicy.stop_on_repeat = True` (`agents/rule_based.py`) karena kebijakan itu deterministik dan tidak memakai info konflik untuk mengubah rencana (bug.md butir 2), sehingga pengulangan tidak mungkin membawa perbaikan. Kebijakan lain (`Random`, IQL, CTDE) tidak mengatur atribut ini, jadi defaultnya tetap `False` dan perilaku `max_rounds` lama tidak berubah untuk mereka.
- **Akibat: `ProcurementEnv("report") + RuleBasedPolicy` sekarang berhenti di putaran 3 (`no_progress`), bukan putaran 6 seperti sebelumnya.** Putaran 1 dan 2 tetap identik dengan jejak yang diverifikasi di `test_rule_based_trace.py` versi lama; putaran 3 mengulang keputusan putaran 2 persis, sehingga terdeteksi sebagai tanpa kemajuan. Angka biaya/kas putaran 1-2 (contoh: total 100.397.000, K1 10.203.000, K2 -20.397.000) tidak berubah — hanya jumlah putaran sebelum berhenti. Ini bukan penyimpangan dari `max_rounds: 6` di `env_default.yaml` (nilainya tetap 6, dipertahankan sebagai batas pengaman simulasi umum); ini penghentian dini khusus untuk kebijakan deterministik yang terbukti mandek.
- **Demo enam-tahap laporan dipindah ke kelas terpisah `ReportReplayEnv` (`src/procurement_marl/report.py`), tidak lagi `ProcurementEnv` langsung.** Untuk mencocokkan istilah "enam tahap koordinasi" di 6.5 (bukan enam putaran environment — lihat catatan istilah yang sudah ada di atas), `ReportReplayEnv`: (a) menonaktifkan `stop_on_repeat` sehingga selalu mencapai `report_revision_required` di akhir tahap koordinasi ke-6 tanpa berhenti dini; (b) untuk putaran ≥2, memproses per-departemen (VMI kedua batch, lalu DA kedua batch, lalu SLM kedua batch) alih-alih per-batch (VMI→DA→SLM→VMI→DA→SLM) seperti `ProcurementEnv` biasa, supaya urutan log cocok dengan tahap 1-6 di `COORDINATION_STAGES`. **Konsekuensi:** jejak yang diuji terhadap Bab 6 (`test_rule_based_trace.py`, `scripts/run_report_trace.py`, kartu "Rule-based (meniru laporan)" + skenario fixture di dashboard) sekarang memverifikasi `ReportReplayEnv`, bukan environment produksi yang dipakai untuk melatih dan mengevaluasi IQL/CTDE. Rumus biaya, kas, dan reward berasal dari fungsi murni yang sama (`costs.py`, `rewards.py`, `_end_round`), jadi angka rupiah tidak terpengaruh; yang berbeda adalah urutan giliran per-batch vs per-departemen dan kapan episode berhenti. `ProcurementEnv` yang dipakai IRE/VMI/DA/SLM untuk training tetap memakai alur per-batch sesuai Fase 2 asli.
- **Kenapa dipertahankan (bukan direvert):** matematika (biaya, diskon, kas, reward) tidak berubah dan tetap lolos semua angka acuan; hanya kapan-berhenti dan bagaimana-jejak-ditampilkan yang berubah, keduanya untuk kasus khusus kebijakan rule-based pada skenario fixture. `RuleBasedPolicy` tetap tidak dipakai untuk melatih IQL/CTDE, jadi ini tidak memengaruhi hasil training atau perbandingan kebijakan pembelajaran.
